...
cv2.createTrackbar("Hue Min","TrackBars_red",130,179,empty)   #トラックバーを作る(実装した結果最も赤をよくマスクできていた値をそれぞれの初期値としている)
cv2.createTrackbar("Hue Max","TrackBars_red",179,179,empty)
...
def detect_red():
    ...
    h_min = cv2.getTrackbarPos("Hue Min", "TrackBars_red")  # トラックバーの位置の値を取得する
    h_max = cv2.getTrackbarPos("Hue Max", "TrackBars_red")
    s_min = cv2.getTrackbarPos("Sat Min", "TrackBars_red")
    s_max = cv2.getTrackbarPos("Sat Max", "TrackBars_red")
    v_min = cv2.getTrackbarPos("Val Min", "TrackBars_red")
    v_max = cv2.getTrackbarPos("Val Max", "TrackBars_red")
