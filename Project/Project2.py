...
def PreProcessing(img):
    ...
    imgTres = cv2.erode(imgDialation,kernel,iterations=1)
...
while True:
    ...
    biggest = getCounters(imgTres)
    if biggest.size != 0:
        warpedIMG = getWarp(img,biggest)
    
        imageArray = ([img,imgContour],[imgTres,warpedIMG])
    else:
        imageArray = ([img,imgContour],[imgTres,img])
        
    stackedimg = stackImages(0.6,imageArray)
